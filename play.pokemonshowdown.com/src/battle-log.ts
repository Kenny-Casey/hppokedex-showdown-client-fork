...
export class BattleLog {
	...
	static createReplayFile(room: { battle: Battle, id?: string, fragment?: string }) {
		...
		buf += '</div>\n';
		buf += '<script>\n';
		buf += `let daily = Math.floor(Date.now()/1000/60/60/24);document.write('<script src="http://localhost:8080/play.pokemonshowdown.com/js/replay-embed.js?version'+daily+'"></'+'script>');\n`;
		buf += '</script>\n';
		return buf;
	}
