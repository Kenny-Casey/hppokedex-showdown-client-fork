...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'rs' | 'frlg' | 'bw1' | 'letsgo' | 'metronome' | 'natdex' |
		...
		'svdlc1natdex' | 'stadium' | 'lc' | 'champions' | 'hppokedex'| null = null;
	isDoubles = false;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | Dex.PokemonSet = '' as ID) {
		...
		//Hppokedex changes
		if (format.includes('hppokedex')){
			this.formatType = 'hppokedex';
			format = format.slice(9) as ID;
			this.dex = Dex.mod('gen9hppokedex' as ID);
		}
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		//Hppokedex changes
		if (this.formatType === 'hppokedex') table = table['gen9hppokedex'];
	...
	getTier(pokemon: Dex.Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			//Hppokedex changes
			this.formatType === 'hppokedex' ? 'gen9hppokedex':
			`gen${gen}`;
		if (table?.[tableKey]) {
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	static readonly GOOD_DOUBLES_MOVES = [
		'allyswitch', 'bulldoze', 'coaching', 'electroweb', 'faketears', 'fling', 'followme', 'healpulse', 'helpinghand', 'junglehealing', 'lifedew', 'lunarblessing', 'muddywater', 'pollenpuff', 'psychup', 'ragepowder', 'safeguard', 'skillswap', 'snipeshot', 'wideguard', 'decorate', 'snarl', 'soullure', 'stickybubbles',
	] as ID[] as readonly ID[];
	getBaseResults() {
		if (!this.species) return this.getDefaultResults();
		const dex = this.dex;
		...
		//Hppokedex changes
		if (this.formatType === 'hppokedex') lsetTable = lsetTable['gen9hppokedex'];
